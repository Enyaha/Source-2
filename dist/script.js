...
var filter = function filter() {
  ...
  var marLovers = wrapper.querySelectorAll('.lovers');
...
var forms = function forms() {
  ...
  form.forEach(function (item) {
    item.addEventListener('submit', function (event) {
      ...
      console.log(api); // item.closest('.popup-design') ? api = path.designer : api = path.question;   JSHint ругается
...
var pictureSize = function pictureSize(imgSelector) {
  var block = document.querySelectorAll(imgSelector);
...
var scrolling = function scrolling(upSelector) {
  ...
  links.forEach(function (link) {
    link.addEventListener('click', function (event) {
      event.preventDefault();
      var wihtTop = document.documentElement.scrollTop;
      var hash = this.hash;
      ...
      function step(time) {
        ...
        var r = toBlock < 0 ? Math.max(wihtTop - progress / speed, wihtTop + toBlock) : Math.min(wihtTop + progress / speed, wihtTop + toBlock);
